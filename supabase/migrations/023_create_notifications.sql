...
-- 同じ通知の重複を防ぐユニーク制約
CREATE UNIQUE INDEX idx_notifications_unique_like ON public.notifications(user_id, actor_id, type, post_id) WHERE type = 'like';
CREATE UNIQUE INDEX idx_notifications_unique_follow ON public.notifications(user_id, actor_id, type) WHERE type = 'follow';
